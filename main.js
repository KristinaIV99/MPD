...
class App {
  ...
  async handleFile(e) {
    try {
      ...
      const markdown = await this.reader.readFile(file);
      this.logger.debug('Failas sėkmingai nuskaitytas');
      this.setContent(markdown);
      this.logger.debug('Markdown turinys sėkmingai įkeltas');
  ...
  setContent(markdown) {
    ...
    div.textContent = markdown; // Tiesiog rodome markdown tekstą
